    ...
    try {
      ...
      if (finalUrl) {
        if (finalUrl.includes('youtube.com/watch')) {
          ...
          if (videoId) finalUrl = `https://www.youtube.com/embed/${videoId}`;
        } else if (finalUrl.includes('youtube.com/live/')) {
          const videoId = finalUrl.split('youtube.com/live/')[1].split('?')[0];
          if (videoId) finalUrl = `https://www.youtube.com/embed/${videoId}`;
        }
      }

      await axios.post('https://cricket-pro-three.vercel.app/api/admin/live-stream', { url: finalUrl });
      setYtLink(finalUrl); 
