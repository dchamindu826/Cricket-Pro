...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // 4. Active Match Check (404 Error එක ආවත් අනිත් ඒවා වැඩ කරන්න try...catch දැම්මා)
        try {
            ...
            console.log("Active Match API තාම Vercel එකේ නෑ. කරුණාකර Backend එක Deploy කරන්න."); 
        }

        // 5. RAPID API: Live Matches ගන්නවා
        try {
            const options = {
              method: 'GET',
  ...
  const handleSetActiveMatch = async () => {
    ...
    try {
      // මෙතනත් Error එක අල්ලලා තියෙන්නේ Backend එක Deploy කරනකම් Crash නොවෙන්න.
      ...
      alert('Failed to update active match. Please make sure the Backend is updated on Vercel.'); 
    }
