  ...
  const handleReact = async (commentId, type) => {
      try {
          ...
          if(response.ok) {
              ...
              // UI එක විතරක් update කරනවා (ආයෙත් fetch කරන්නේ නෑ ලෝඩ් එක අඩු කරන්න)
    ...
    <div id="posts" className="max-w-4xl mx-auto px-4 my-10 pt-10">
      ...
      <div className="bg-[#0b1b36] rounded-2xl border border-slate-700 overflow-hidden shadow-2xl">
        ...
        <div className="p-6 relative z-20 -mt-16">
          ...
          <h2 className="text-2xl md:text-3xl font-extrabold text-white mb-2 drop-shadow-md">{activePost.title}</h2>
          
          {/* අලුත් Description එක පෙන්වීම */}
          {activePost.description && (
             <p className="text-slate-300 text-sm mb-6 bg-slate-800/50 p-3 rounded-lg border border-slate-700">{activePost.description}</p>
          )}

          {/* අලුත් Prizes ටික ලස්සනට පෙන්වීම ($ sign එක්ක) */}
          {parsedPrizes.length > 0 && (
              <div className="mb-6 flex flex-wrap gap-3">
                 {parsedPrizes.map((prize, idx) => (
                    <div key={idx} className="bg-gradient-to-r from-yellow-500/10 to-cricket-gold/10 border border-cricket-gold/40 px-4 py-2 rounded-xl flex items-center gap-2">
                       <FiAward className="text-cricket-gold" size={20}/>
                       <div>
                          <p className="text-[10px] text-yellow-500 uppercase font-bold">{prize.title}</p>
                          <p className="text-white font-black">${prize.price}</p>
                       </div>
                    </div>
                 ))}
              </div>
      ...
      <div className="mt-10 bg-[#0b1b36]/80 p-6 rounded-2xl border border-slate-700/50 shadow-xl">
          ...
          <div className="h-[400px] overflow-y-auto pr-3 space-y-4 custom-scrollbar">
              {comments.length > 0 ? comments.map((comment, index) => {
                  ...
                  const reacts = comment.reactions || {}; // ගත්ත රිඇක්ෂන්ස්
                  
                  return (
                  <div key={comment.id} className={`flex flex-col gap-2 p-5 rounded-2xl border transition-all ${
                      ...
                      <div className="flex gap-4">
                        <div className="relative shrink-0">
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center font-black text-lg ${comment.is_vip ? 'bg-gradient-to-br from-cricket-gold to-yellow-600 text-black' : isFirstComment ? 'bg-emerald-500 text-white' : 'bg-slate-700 text-white'}`}>
                                {comment.avatar}
                            </div>
                        </div>
                        ...
                        <div className="flex-1">
                            ...
                            <div className="flex items-center gap-3 mt-3">
                               <button onClick={() => handleReact(comment.id, 'like')} className="flex items-center gap-1 text-slate-500 hover:text-neon-blue transition text-xs font-bold">
                                  <FiThumbsUp/> {reacts.like || 0}
                               </button>
                               <button onClick={() => handleReact(comment.id, 'love')} className="flex items-center gap-1 text-slate-500 hover:text-pink-500 transition text-xs font-bold">
                                  <FiHeart/> {reacts.love || 0}
                               </button>
                            </div>
                        </div>
                      </div>

                      {/* Admin Reply Highlight වෙලා පේන කෑල්ල */}
                      {comment.admin_reply && (
                          <div className="mt-2 ml-16 bg-gradient-to-r from-cricket-gold/20 to-transparent border-l-4 border-cricket-gold p-3 rounded-r-lg">
                              <div className="flex items-center gap-2 mb-1">
                                  <FaCrown className="text-cricket-gold" size={14}/>
                                  <span className="text-cricket-gold text-xs font-black uppercase tracking-wider">Admin Reply</span>
                              </div>
                              <p className="text-slate-200 text-sm font-medium leading-relaxed">{comment.admin_reply}</p>
                          </div>
                      )}
...
// මේ icon එක උඩ import එකට දාන්න අමතක වුණා නම්
