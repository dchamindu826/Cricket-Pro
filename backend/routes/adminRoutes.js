...
const axios = require('axios'); // <-- අලුතින් එකතු කලා
...
// ==========================================
// RAPID API ROUTES (අලුතින් එකතු කල කොටස)
// ==========================================

const RAPIDAPI_KEY = 'cd72733c17mshb6183f2ce7d960ap15870fjsn5d09d19ac6a5';
const RAPIDAPI_HOST = 'cricbuzz-cricket.p.rapidapi.com';

let matchesCache = { data: null, lastFetched: 0 };
let scoreCache = {}; 
...
router.get('/live-matches', async (req, res) => {
  try {
    const now = Date.now();
    if (matchesCache.data && (now - matchesCache.lastFetched < CACHE_DURATION)) {
    ...
    matchesCache = { data: response.data, lastFetched: now };
    res.json(response.data);
  } catch (error) {
    res.status(500).json({ message: "Error fetching live matches from RapidAPI" });
  }
});

router.get('/live-score/:matchId', async (req, res) => {
  const { matchId } = req.params;
  try {
    const now = Date.now();
    
    // Cache eke data thiyenawanm eka return karanawa
    if (scoreCache[matchId] && (now - scoreCache[matchId].lastFetched < CACHE_DURATION)) {
      return res.json(scoreCache[matchId].data);
    }

    // API eka call karanna ona options tika
    const options = {
      method: 'GET',
    ...
    const response = await axios.request(options);
    scoreCache[matchId] = { data: response.data, lastFetched: now };
    res.json(response.data);
    
  } catch (error) {
    // 🔴 Vercel logs walata hariyatama error eka print kirima
    console.error("RapidAPI Error:", error.response ? error.response.data : error.message);
    res.status(500).json({ message: "Error fetching score from RapidAPI" });
  }
});
